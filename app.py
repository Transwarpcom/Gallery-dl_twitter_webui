...
def parse_timestamp(
    ...
    json_file = next((f for f in files_for_post if f.endswith('.json')), None)
    if json_file:
        json_path = os.path.join(user_folder_path, json_file)
        try:
            with open(json_path, 'r', encoding='utf-8') as f:
                ...
                if date_str:
                    try:
                        ...
                        app.logger.debug(f"JSON 文件 '{json_file}' 的 'date' 字段 '{date_str}' 对于帖子 {post_id} 不是 YYYY-MM-DD HH:MM:%S 格式。")
                        pass
        except json.JSONDecodeError as e:
...
def process_and_cache_user_posts(username: str, force_rescan: bool = False) -> Tuple[int, int]:
    ...
    if force_rescan:
        ...
        if db_user.posts: # 这会正确检查 InstrumentedList 是否为空
    ...
    for tweet_id, associated_files in files_by_tweet_id.items():
        ...
        if json_file_path and os.path.exists(json_file_path):
            try:
                with open(json_file_path, 'r', encoding='utf-8') as f:
                    ...
                    if not temp_post_data['text_content'] and ('full_text' in json_data or 'text' in json_data):
                        temp_post_data['text_content'] = json_data.get('full_text') or json_data.get('text')
            except json.JSONDecodeError as e:
                ...
                with open(txt_path, 'r', encoding='utf-8') as f:
                    ...
                    if lines:
                        ...
                        try:
                            ...
                            if not temp_post_data['text_content']: # 如果文本内容尚未从 JSON 设置，且第一行不是日期，则使用它
        ...
        # 调用 parse_timestamp 以根据新的优先级获取最准确的时间
